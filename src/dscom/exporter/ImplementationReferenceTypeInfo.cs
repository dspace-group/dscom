...
internal sealed class ImplementationReferenceTypeInfo : ReferenceTypeInfo
{
    ...
    {
        typeInfoUsingType.GetImplTypeFlags(index, out var pImplTypeFlags);

        if (pImplTypeFlags != 0)
        {
            ImplementationFlags = pImplTypeFlags;
        }
    }

    public IMPLTYPEFLAGS? ImplementationFlags { get; private set; }
